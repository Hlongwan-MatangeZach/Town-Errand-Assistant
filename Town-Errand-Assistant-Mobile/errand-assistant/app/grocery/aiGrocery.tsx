...
export default function AiGroceryScreen() {
  ...
  const [focusedInput, setFocusedInput] = useState<"budget" | "family" | null>(
    null,
  );
  ...
  const handleGenerateList = async () => {
    ...
    try {
      ...
      const dietaryNames = selectedDietaryRestrictions.map(
        (id) => DietaryRestrictions.find((d) => d.id === id)?.label || id,
      );
      const categoryNames = selectedCategories.map(
        (id) => ShoppingCategory.find((c) => c.id === id)?.name || id,
      );
      const mealFocusLabels = selectedMealFocus
        .map((id) => MEAL_FOCUS.find((m) => m.id === id)?.label || id)
        .join(", ");
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.essentialWrapper}>
            <View
              style={[
                ...
                focusedInput === "budget" && styles.inputContainerActive,
              ]}
            >
              <Ionicons
                name="wallet-outline"
                ...
                color={
                  focusedInput === "budget"
                    ...
                    : themes.light.colors.textSecondary || "#666"
                }
              />
              <TextInput
            ...
            <View
              style={[
                styles.inputContainer,
                focusedInput === "family" && styles.inputContainerActive,
              ]}
            >
              <Ionicons
                name="people-outline"
        ...
        <View style={styles.categoryContainer}>
          ...
          >
            ...
            {Array.from({
              length: Math.ceil((ShoppingCategory.length - 1) / 2),
            }).map((_, colIndex) => (
              <View key={`col-${colIndex}`} style={{ gap: 5 }}>
                {ShoppingCategory.slice(
                  1 + colIndex * 2,
                  1 + colIndex * 2 + 2,
                ).map((cat) => {
                  const isSelected = selectedCategories.includes(cat.id);
        ...
        <View style={styles.diertyContainer}>
          ...
          >
            {DietaryRestrictions.map((restriction) => {
              const isSelected = selectedDietaryRestrictions.includes(
                restriction.id,
              );
              return (
                <Pressable
